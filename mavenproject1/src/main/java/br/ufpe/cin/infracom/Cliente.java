...
public class Cliente extends javax.swing.JFrame {
    ...
    private void enviarNumPacotes() throws SocketException, UnknownHostException, IOException{
        ...
        for (int i = 0; i < qtePacotes; i++) {
            if(i == qtePacotes - 1){
                enviarDados = this.inserirCabecalho(enviarDados, true, 1, qte);
            }else{
                enviarDados = this.inserirCabecalho(enviarDados, false, 1, qte);
            }
    ...
    private void enviarDuracaoTeste() throws SocketException, UnknownHostException, IOException{
        ...
        for (int i = 0; temTempo; i++) {
            if((System.currentTimeMillis() - tempo) >= duracao){
                temTempo = false;
                enviarDados = this.inserirCabecalho(enviarDados, true, 2, i);
            }else{
                enviarDados = this.inserirCabecalho(enviarDados, false, 2, i);
            }
            DatagramPacket enviarPacote = new DatagramPacket(enviarDados, enviarDados.length, ipServidor, Integer.parseInt(this.portaDestino.getText()));
            clientSocket.send(enviarPacote);
        }
    ...
    private void enviarTotalBytes() throws SocketException, UnknownHostException, IOException{
        ...
        for (int i = 0; i < x; i++) {
            if(i == x - 1){
                enviarDados = this.inserirCabecalho(enviarDados, true, 3, Integer.parseInt(this.totalBytes.getText()));
            }else{
                enviarDados = this.inserirCabecalho(enviarDados, false, 3, Integer.parseInt(this.totalBytes.getText()));
            }
            DatagramPacket enviarPacote = new DatagramPacket(enviarDados, enviarDados.length, ipServidor, Integer.parseInt(this.portaDestino.getText()));
            clientSocket.send(enviarPacote);
        }
    ...
    private byte[] inserirCabecalho(byte[] dados, boolean ultimo, int opcao, int qte){
        byte versao;
